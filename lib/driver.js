...
export class XCUITestDriver extends BaseDriver {
  ...
  async stop() {
    ...
    if (this.wda?.fullyStarted) {
      ...
      // If the session skipped the xcodebuild (this.wda.canSkipXcodebuild), or the
      // closeWDAOnSessionEnd is true, the this.wda instance should quit properly.
      if (
        (!this.wda.webDriverAgentUrl && this.opts.useNewWDA) ||
        this.opts.closeWDAOnSessionEnd ||
        this.wda.canSkipXcodebuild
      ) {
        await this.wda.quit();
      }
    }
